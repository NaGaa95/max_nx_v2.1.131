 ...
 *
 * 2.1.131 update: supports loading multiple modules (libc++_shared.so +
...
static uintptr_t so_resolve_symbol(so_module *mod, DynLibFunction *funcs, int num_funcs, const char *name) {
  // Check the exports of every other loaded module first, so that libGame's
  // std::/__cxa_/RTTI data references resolve to libc++_shared.
