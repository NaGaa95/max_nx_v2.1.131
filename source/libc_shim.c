...
//
// before reading anything from the .ras archives, the engine probes for a
// loose file override on disk -- thousands of probes per boot, and every
// miss is a slow FAT directory lookup on the SD card. data/ and es2/ are
// immutable game data, so we walk them once into a sorted list and answer
// the probes from memory. paths outside those trees (settings.bin etc.,
// which the game creates at runtime) always go to the filesystem.
// ---------------------------------------------------------------------------
